    ...
    <div className="flex flex-col sm:flex-row gap-6 mt-8 text-justify">
      ...
      <div className="flex flex-col w-2/2 sm:w-2/3">
        <div data-aos="fade-left" data-aos-delay={animationDelays.POSITION}>
          <p className="text-xl md:text-2xl font-bold">{position}</p>
          <p className="text-lg md:text-xl mt-2">{company}</p>
        </div>
        <ul>
          <li className="px-0 pt-2">
            {description && (
              <div
                data-aos="fade-left"
                data-aos-delay={animationDelays.DESCRIPTION}
              >
                {description}
              </div>
            )}
            <ul className="list-disc">
              {formattedAchievements.map((achievement, index) => (
                <li
                  key={index}
                  className="ml-5 pt-2 pr-0 text-sm md:text-md"
                  data-aos="fade-left"
